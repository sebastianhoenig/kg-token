...
class MovieLens:
    ...
    def create_graph(self):
        ...
        unique_index = torch.arange(data_train["user"].num_nodes, device=self.device, dtype=torch.long).unsqueeze(1)
        ...
        unique_index = torch.arange(data_test["user"].num_nodes, device=self.device, dtype=torch.long).unsqueeze(1)
