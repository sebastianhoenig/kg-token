...
class GraphTokenLLM(nn.Module):
    ...
    def forward_regression(self, batch, graph):
        attention_masks = []
        user_ids = []
        movie_ids = []
        target_masks = []
        ...
        # Compute MSE loss only for the relevant values
        loss_fn = torch.nn.MSELoss()
        loss = loss_fn(relevant_logits, relevant_labels)

        return loss, relevant_logits, relevant_labels
