...
def train_gnn(config: Any):
    wandb.init(project=f"{config.project_name}", name=f"{config.gnn_model}-{config.gnn_hidden_dim}-{config.gnn_num_layers}-{config.gnn_dropout}-{config.gnn_aggr}", config=config)
    apply_seed(0)
