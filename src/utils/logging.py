...
def log_to_wandb(res, epoch, example_ct, loss, optimizer):
    ...
    wandb.log({"epoch": epoch, "yes_preds": correct_yes_preds+wrong_yes_preds, "wrong_yes_preds": wrong_yes_preds}, step=example_ct)
    wandb.log({"epoch": epoch, "no_preds": correct_no_preds+wrong_no_preds, "wrong_no_preds": wrong_no_preds}, step=example_ct)
